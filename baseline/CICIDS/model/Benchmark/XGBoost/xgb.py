...
def find_optimal_threshold(model, X_val, y_val, unknown_data, normal_data):
    ...
    # 尝试不同阈值
    thresholds = np.linspace(0.0001, 0.5000, 5000)
    results = []
    ...
    for threshold in thresholds:
        ...
        results.append((threshold, unknown_cov, accuracy, precision, recall, f1, fpr, score))
    
    # 找到最佳阈值
    best_result = max(results, key=lambda x: x[7])
    return best_result
...
def main():
    ...
    for unknown_type in unknown_types:
        ...
        # 训练模型
        print("训练XGBoost模型...")
        model, training_time = train_model(X_train_split, y_train_split, n_estimators=100, learning_rate=0.1, max_depth=3)
        ...
        best_threshold, best_acc, best_pre, best_recall, best_f1, best_unknown_cov, best_fpr, best_score = best_result
        
        with open(f'{unknown_type}_best_threshold_results.txt', 'w') as f:
            f.write(f"Best Threshold: {best_threshold:.6f}\n")
            f.write(f"Unknown Coverage at Best Threshold: {best_unknown_cov:.4f}\n")
            f.write(f"Accuracy at Best Threshold: {best_acc:.4f}\n")
            f.write(f"Precision at Best Threshold: {best_pre:.4f}\n")
            f.write(f"Recall at Best Threshold: {best_recall:.4f}\n")
