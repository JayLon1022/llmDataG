...
def initialize_particles(num_particles, dim):
    positions = np.random.uniform(0, 1, (num_particles, dim)) 
    velocities = np.random.uniform(VELOCITY_RANGE[0], VELOCITY_RANGE[1], (num_particles, dim))
    pbest = positions.copy() 
    gbest = np.random.uniform(0, 1, dim) 
    ...
    while coverage < EXPECTED_COVERAGE and iteration < MAX_ITERATIONS:
        for i in range(num_particles):
            ...
            if fit_value > SELF_RADIUS: 
                detector_radius = fit_value - SELF_RADIUS 
...
def mutate_detectors(detectors, radii, self_data, perturbation_factor):
    ...
    for detector, radius in zip(detectors, radii):
        ...
        if fit_value > SELF_RADIUS:
            ...
            mutated_radii.append(fit_value - SELF_RADIUS) 
