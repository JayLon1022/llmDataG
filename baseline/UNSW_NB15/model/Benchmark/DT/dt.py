...
# 训练决策树模型
def train_model(X_train, y_train, max_depth=None, min_samples_split=2, min_samples_leaf=1):
    # 创建决策树分类器
...
def main():
    ...
    for unknown_type in unknown_types:
        ...
        # 训练模型
        print("训练决策树模型...")
        model, training_time = train_model(X_train_split, y_train_split, max_depth=10)
        ...
        # 将结果保存为文本文件
        with open(f'{unknown_type}/dt_results.txt', 'w') as f:
            for key, value in results.items():
                if key != 'confusion_matrix':
                    f.write(f"{key}: {value}\n")
            f.write(f"confusion_matrix:\n{conf_matrix}\n")
        
        print(f"结果已保存到 {unknown_type}/dt_results.txt")
