...
def plot_heatmap(unknown_type):
    # 读取结果
    results = pd.read_csv(f'ablation_results_{unknown_type}.csv')
    
    # 创建F1分数的热力图
    pivot_f1 = results.pivot(index='self_radius', columns='sample_size', values='f1')
    plt.figure(figsize=(10, 8))
    sns.heatmap(pivot_f1, annot=True, fmt='.2%', cmap='YlOrRd')
    plt.title(f'F1 Score Heatmap for {unknown_type}')
    plt.savefig(f'f1_heatmap_{unknown_type}.png')
    plt.close()
    
    # 创建FAR的热力图
    pivot_far = results.pivot(index='self_radius', columns='sample_size', values='far')
    plt.figure(figsize=(10, 8))
    sns.heatmap(pivot_far, annot=True, fmt='.2%', cmap='YlOrRd_r')
    plt.title(f'False Alarm Rate Heatmap for {unknown_type}')
    plt.savefig(f'far_heatmap_{unknown_type}.png')
    plt.close()
...
def main():
    # 定义要测试的参数组合
    self_radii = np.arange(0.001, 0.101, 0.001).tolist() 
    self_count = list(range(10, 101, 10))
    ...
    for unknown_type in unknown_types:
        ...
        for radius in self_radii:
            results[unknown_type][radius] = {}
            for sample_size in self_count:
                # 加载并处理数据
                train_data, test_data, unknown = load_data(train_self_path, train_nonself_path, 
                                                         test_self_path, test_nonself_path, unknown_path)
                ...
                # 获取指定数量的自体样本
                self_data = X_train[y_train == 0][:sample_size]
                ...
                # 存储结果
                results[unknown_type][radius][sample_size] = {
                    'accuracy': acc,
                    'precision': precision,
                    'recall': recall,
                    'f1': f1,
                ...
                print(f"Type: {unknown_type}, Radius: {radius}, Sample Size: {sample_size}")
                print(f"F1: {f1:.2%}, FAR: {far:.2%}, UC: {uc:.2%}")
        
        # 保存结果到CSV文件
        results_df = pd.DataFrame()
        for radius in self_radii:
            for sample_size in self_count:
                metrics = results[unknown_type][radius][sample_size]
                results_df = results_df.append({
                    ...
                    'far': metrics['far'],
                    'uc': metrics['uc'],
                    'tolerance_time': metrics['tolerance_time']
                }, ignore_index=True)
        
        # 保存结果
        results_df.to_csv(f'ablation_results_{unknown_type}.csv', index=False)

    for unknown_type in ["dos", "probe", "r2l", "u2r"]:
        plot_heatmap(unknown_type)
